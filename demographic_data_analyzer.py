...
def calculate_demographic_data(print_data=True):
    ...
    educationNum = len(df["education"])
    # What is the percentage of people who have a Bachelor's degree?
    percentage_bachelors = round((len(df[df["education"] == "Bachelors"])/educationNum)*100, 1)
    ...
    higher_education_rich = round((len(higher_education[higher_education['salary'] == '>50K'])/len(higher_education["salary"]))*100, 1)
    lower_education_rich = round((len(lower_education[lower_education['salary'] == '>50K'])/len(lower_education["salary"]))*100, 1)
